...
export default function Employees({ token }) {
  ...
  async function assign() {
    await axios.post(`${API}/api/employees/assign`,
      { employeeId: assignEmployee, teamIds: selectedTeams },
      { headers: { Authorization: 'Bearer ' + token } }
    );
        ...
        <tbody>
          {list.map(it => (
            <tr key={it.id}>
              ...
              <td>{it.role}</td>
              <td>
                <button onClick={() => { setAssignEmployee(it.id); }}>Assign</button>
      ...
      {assignEmployee && (
        <div style={{ marginTop: 12 }}>
          ...
          <div>
            ...
            <button onClick={() => setAssignEmployee(null)}>Cancel</button>
          </div>
        </div>
      )}
    </div>
